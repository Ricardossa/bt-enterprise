...
validate_full() {
    ...
    archive_has 'database/banco_template.db' || fail 'FULL sem banco_template.db.'
    ...
    if archive_matches '(^|/)(database/banco\.db|temp_client\.db|\.git/|99_quarentena/|logs/|cache/|uploads/|runtime/|service/)|\.(old|bak|bkp|exe|bat|vbs)$'; then
...
install_dependencies() {
    info 'Instalando dependências Apache/PHP.'
    require_executable /usr/bin/apt-get apt
    /usr/bin/apt-get update -y
    DEBIAN_FRONTEND=noninteractive /usr/bin/apt-get install -y apache2 libapache2-mod-php php-cli php-sqlite3 php-curl php-mbstring php-gd php-zip php-xml sqlite3 curl unzip
    validate_runtime_tools
    php_has_module pdo_sqlite || fail 'Extensão pdo_sqlite indisponível; instale o pacote php-sqlite3.'
...
new_install() {
    ...
    prepare_runtime
    /usr/bin/install -o www-data -g www-data -m 664 "$APP_DIR/database/banco_template.db" "$APP_DIR/database/banco.db"
    configure_apache
    configure_sync_service
