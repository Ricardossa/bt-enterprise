...
check("PDO SQLite", extension_loaded('pdo_sqlite'));
check("CURL", extension_loaded('curl'));

// 2. Banco e Permissões
$dbPath = dirname(__DIR__) . '/database/banco.db';
check("Banco de Dados", file_exists($dbPath));
check("Permissão Escrita", is_writable(dirname($dbPath)));
